...
namespace AdventOfCode.DaySeven
{
    ...
    public class PartTwo_wrong
    {
        ...
        public static long GetAnswer(string fileName)
        {
            ...
            var hands = new Dictionary<string, CardPartTwo>();
            foreach (var line in lines)
            {
                var seperated = line.Split(' ');
                hands.Add(seperated.First(), new CardPartTwo() { Bet = Int32.Parse(seperated.Last()), Type = 0, Rank = 0, Value = ValueCard(seperated.First()) });
            }
            ...
            foreach (var hand in hands)
            {
                ...
                // Get type
                if (cardCounts.Values.Max() + hand.Key.Count(x => x == 'J') >= 5)
                {
                    hands[hand.Key].Type = 0;
                }
                else if (cardCounts.Values.Max() + hand.Key.Count(x => x == 'J') == 4)
                {
                    hands[hand.Key].Type = 1;
                }
                else if (cardCounts.Values.Max() + hand.Key.Count(x => x == 'J') == 3 && cardCounts.Values.Any(x => x == 2))
                {
                    hands[hand.Key].Type = 2;
                }
                else if (cardCounts.Values.Max() + hand.Key.Count(x => x == 'J') == 3)
                {
                    hands[hand.Key].Type = 3;
                }
                else if (cardCounts.Values.Where(x => x + hand.Key.Count(x => x == 'J') == 2).Count() == 2)
                {
                    hands[hand.Key].Type = 4;
                }
                else if (cardCounts.Values.Max() + hand.Key.Count(x => x == 'J') == 2)
                {
